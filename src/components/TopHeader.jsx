  ...
  return (
    <div className={`${scroll ? 'lg:container lg:mx-auto w-full flex items-center justify-around py-5 sticky top-0 z-50 bg-black rounded-lg' : 'lg:container lg:mx-auto w-full flex items-center justify-around py-5 sticky top-0 z-50'}`}>
      ...
      <span className="w-auto md:w-2/6 flex items-center justify-start md:justify-end cursor-pointer p-1 relative  z-50" >
        <span className=' w-16 mr-1 lg:w-40 h-10 shadow-md bg-black/70 flex items-center justify-center rounded-md' onClick={() => setShowMenu(!showMenu)}>
          {
            !firebase.url ? <img src="/user.png" alt="userPic" className='w-9 h-5/6 object-cover rounded-lg ' /> : <img src={firebase.url} alt="userPic" className='w-9 h-5/6 object-cover rounded-lg ' />
          }

          {!firebase.name ? <h2 className='text-[10px] text-white mx-2 font-semibold lg:flex hidden'>{firebase.email}</h2> : <h2 className='text-xs text-white mx-2 font-semibold lg:flex hidden'>{firebase.name}</h2>}
