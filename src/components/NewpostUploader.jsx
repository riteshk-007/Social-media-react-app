  ...
  return (
    <div className="w-full lg:w-4/5  rounded-xl px-3 py-2  bg-black/20  flex items-center justify-center flex-col">
        <span className="flex items-center justify-center w-full">
                {!firebase.url ?<img src="/user.png" alt="" className="lg:w-12 bg-black lg:h-12 w-10 h-10 rounded-2xl border-2 border-gray-500 object-cover overflow-hidden"/> :
                <img src={firebase.url} alt="" className="lg:w-12 lg:h-12 w-10 h-10 rounded-2xl border-2 border-gray-500 object-cover overflow-hidden"/> }
        ...
        <span className="flex items-center justify-end w-full lg:w-4/5 my-3">
                <label className='flex items-center justify-center  bg-black/20 px-3 rounded-lg cursor-pointer py-2  lg:mr-8' onClick={()=> setUploadData(true)}>
     ...
     uploadData &&  
      <div className='fixed w-screen h-screen top-0 left-0 bg-black/50 z-50 flex items-center justify-center'>
        ...
        <div className='md:w-1/4  bg-gray-700 relative shadow-lg rounded-md flex items-center justify-center flex-col text-white px-3' >
                <input type="file" id='image' accept="image/*"  className='my-3 cursor-pointer' onChange={(e)=> setCover(e.target.files[0])} required/>
                <input type="text"  maxLength={150} placeholder='post title....'
                ...
                <input type='submit' className='bg-yellow-500 px-5 py-2 my-4 text-white font-semibold rounded-md cursor-pointer' value={'Upload'} />
        <ImCross fontSize={15} className='absolute top-2 right-2 cursor-pointer' onClick={()=>setUploadData(false)}/>
        </div>
        </form>
     </div>
        }
